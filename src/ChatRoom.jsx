...
import { getRoomMessages, publishKey, getRoomKeys } from "./api";
...
export default function ChatRoom({ room, token, userId, username, onBack }) {
  ...
  async function refreshSharedKeys() {
    ...
    const keys = await getRoomKeys(room.id, token);
  ...
  async function tryDecrypt(recipients, senderId) {
    if (!recipients || Object.keys(recipients).length === 0) return "[no content]";

    if (recipients.solo) {
      try {
        return decodeURIComponent(escape(atob(recipients.solo.ciphertext)));
      } catch {
        return "[solo decrypt failed]";
      }
    }

    const myIdStr = String(userId);
    const mySlice = recipients[myIdStr];
  ...
  useEffect(() => {
    ...
    setupEncryption().then(async () => {
      if (cancelled) return;
      const msgs = await getRoomMessages(room.id, token);
      if (cancelled) return;
  ...
  async function handleSend(e) {
    ...
    if (peers.length === 0) {
      recipients = {
        solo: { ciphertext: btoa(unescape(encodeURIComponent(text))), iv: "none" }
      };
    } else {
      recipients = {};
